...
import static com.example.laowai.bt_with_pref.pref_simple.my_settings_xxx;
        ...
        public boolean onNavigationItemSelected(@NonNull MenuItem item) {
            switch (item.getItemId()) {
                case R.id.navigation_home:
                    ...
                    Intent i = new Intent(getBaseContext(), pref_simple.class);
                    //i.putExtra("PersonID", personID);
                    startActivity(i);
                    return true;
                case R.id.navigation_dashboard:

                    int got_it = my_functions.add_f();
                    Toast.makeText(getApplicationContext(), "xxx"+got_it, Toast.LENGTH_SHORT).show();
                    return true;
                case R.id.navigation_notifications:
                    mTextMessage.setText(R.string.title_notifications);
                    hold_x++;
                    int ttt = my_functions.times_3(hold_x);
                    mTextMessage.setText(String.valueOf(ttt));
                    return true;
            }
            return false;
        }
